# Main function to loop through all instance folders (Gurobi version)
import argparse
import os
...
from src.mip_gurobi import RoutingMIPSolverGurobi
...
def write_solution_to_csv(inst, solution: dict[Courier, list[Delivery]], override=False):
    ...
    output_file = f"solutions/{inst['instance_name']}_gurobi.csv"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Process couriers, deliveries, and travel time matrices from multiple instances using Gurobi solver.")
    ...
    print()
    print("Step 2: Optimizing using Gurobi solver (1800 seconds per instance)...")
    print("-"*70)
    ...
    for inst in all_instance_data:
        ...
        print(f"  {inst['instance_name']} (size: {instance_size})", end="", flush=True)
        try:
            optimizer = RoutingMIPSolverGurobi(couriers=inst['couriers'], deliveries=inst['deliveries'], transit_times=inst['travel_time'])
