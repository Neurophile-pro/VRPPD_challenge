...
class RoutingMIPSolverSCIP:
    ...
    def create_model(self):
        ...
        for i in range(self.num_nodes - 1):
            model.addCons(quicksum(next_node[i, j] for j in range(self.num_nodes)) == 1)
        ...
        for j in range(self.num_couriers, self.num_nodes - 1):
            model.addCons(quicksum(next_node[i, j] for i in range(self.num_nodes)) == 1)
        ...
        for i in range(self.num_nodes):
            for j in range(self.num_nodes - 1):
                ...
                for c in range(self.num_couriers):
                    ...
                    # Formulation: couriers[j,c] >= couriers[i,c] - (1 - next_node[i,j])
                    #             couriers[i,c] >= couriers[j,c] - (1 - next_node[i,j])
    ...
    def optimize(self):
        ...
        model.setParam('limits/time', 1800)  # set time limit, in seconds
        model.optimize()
